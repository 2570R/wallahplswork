...
float Chassis::getInputVertical(float horizontalInput, float verticalInput) {
    ...
    if (chassisDriverSettings.useHalfCircleInput) return copysign(dist(horizontalInput, verticalInput, 0, 0), verticalInput);
...
float Chassis::getInputHorizontal(float horizontalInput, float verticalInput) {
    ...
    if (chassisDriverSettings.useHalfCircleInput) return copysign(dist(horizontalInput, verticalInput, 0, 0), horizontalInput);
...
void Chassis::spinTank(float leftInput, float rightInput, bool useVolt, bool reverse) {
    ...
    if (!reverse) {
        chassisGeometry.leftMotors->spin(fwd, leftInput, volt);
        chassisGeometry.rightMotors->spin(fwd, rightInput, volt);
    }
    else {
        chassisGeometry.leftMotors->spin(fwd, -rightInput, volt);
        chassisGeometry.rightMotors->spin(fwd, -leftInput, volt);
    }
}
...
void Curvature::controllerFeedbackSpin(bool reverse) {
    ...
    steering = clamp(steering * 90.0 / 127.0, -90, 90);

    if (fabs(steering) == 90) spinTank(copysign(outputPower, steering), copysign(outputPower, -steering), false, reverse);
    else {
        std::pair<float, float> outputs = scaleToRatio(outputPower, 2 + tan(deg2rad(steering)) / 2.0, 2 - tan(deg2rad(steering)) / 2.0);
